...
% This script contains initial conditions for and generates the desired trajectory for the new task control scenario
...
thetai = [pi/2,pi/4,-pi/4,-pi/2,0]';    % joint angles     (R^5)
...
Tce_s = Ty( 0, 0, 0.25, 2*pi/3);          % standoff  configuration of the end-effector   wrt (cube frame)
Tce_g = Ty( 0.01, 0,  0.02, 2*pi/3);     % grasping  configuration of the end-effector   wrt (cube frame)
Tsc_i = Tz( 0,  1,  0,  0);            % initial   configuration of the cube           wrt (space frame)
Tsc_f = Tz( 0,  1,  0, -pi);         % final     configuration of the cube           wrt (space frame)
