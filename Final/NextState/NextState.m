...
% q0 = [0,0,0]';                  % chasis configuration
% u0 = [0,0,0,0]';                % wheel angles
% theta0 = [0,0,0,0,0]';          % joint angles
...
% % Get the next state
% [q_next, theta_next, u_next] = NextState(q0,u0,theta0,du,dtheta,dt,speed_max);

% Enforce wheel velocity limits
for i = 1:4
    if(abs(du(i)) > speed_max(6))
        du(i) = speed_max(6)*sign(du(i));
    end
end
...
for i = 1:5
    if(abs(dtheta(i)) > speed_max(i))
        dtheta(i) = speed_max(i)*sign(dtheta(i));
    end
end
